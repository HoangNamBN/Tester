    ...
    <div class="container" style="margin-top: 20px;margin-left:5px;">
        <div class="row">
            ...
            <div class="col-md-2"><a href="Add_Edit_Update_NhanVien.php" class="btn btn-danger" role="button"
                    style="margin-left:700px;width:157px;height:40px;padding-bottom:20px;background:#f86c6b">Thêm Nhân Viên</a></div>
        </div>
                ...
                <?php
                    ...
                    if (isset($_REQUEST['Ok'])) 
                    {
                        ...
                        else
                        {
                            ...
                            if ($num_row > 0 && $MaNV != "") 
                            {
                                echo "$num_row Kết Quả Trả Về Với Từ Khoá <b>$MaNV</b>";
                                while ($row = mysqli_fetch_assoc($query)) {
                                    echo '<tr style=" text-align: center;">
                                      <td>'.$row['MaNV'].'</td>
                                      <td><img src="../Image/'.$row['AnhNV'].'"  style="width:70px;height:80px"></td>
                                      <td>'.$row['TenNV'].'</td>
                                      <td>'.$row['NgaySinh'].'</td>
                                      <td>'.$row['DiaChi'].'</td>
                                      <td>'.$row['SDT'].'</td>
                                      <td>'.$row['TaiKhoan'].'</td>
                                      <td><button class="btn btn-warning" onclick=\'window.open("Add_Edit_Update_NhanVien.php?id='.$row['MaNV'].'","_self")\'>Edit</button></td>
                                      <td><button class="btn btn-info" onclick=\'window.open("XoaNhanVien.php?id='.$row['MaNV'].'","_self")\'>Delete</button></td>
                                  </tr>'; 
                                }
                            } 
                            else {
